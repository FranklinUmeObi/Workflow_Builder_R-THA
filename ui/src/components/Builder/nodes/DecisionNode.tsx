...
import { useNodeEditing, useDecisionCondition } from "../../../hooks";
...
export const DecisionNode: React.FC<DecisionNodeProps> = ({
  ...
  const { label, handleLabelChange, handleKeyDown, updateNodeData } = useNodeEditing(id, data);
  const { condition, showCondition, setShowCondition, updateCondition } = useDecisionCondition(
    id,
    data,
    updateNodeData,
  );
        ...
        <div className="w-full space-y-2">
          ...
          {showCondition && (
            <div className="space-y-2 p-2 bg-white rounded border border-blue-200">
              <div className="text-xs font-medium text-blue-700">Condition</div>
